...
def win32_excel_save(data: list, sheet_name):
    wb = winclient.GetObject(None, "Excel.Application")
    ws = wb.Sheets(SHEET_MAPPING.get(sheet_name))
    next_row = ws.UsedRange.Rows.Count + 1
    ws.Range(f"A{next_row}:E{next_row}").Value = [data]
...
class Listener(tweepy.StreamingClient):
    ...
    def on_data(self, raw_data):
        ...
        win32_excel_save(data, sheet_name)
